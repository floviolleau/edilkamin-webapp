...
>(
    ...
    const i18nextInstance = await initI18next(locale, Array.isArray(ns) ? ns as string[] : ns as string);

    return {
        t: Array.isArray(ns) ? i18nextInstance.getFixedT(locale, ns[0], options.keyPrefix) : i18nextInstance.getFixedT(locale, ns as FlatNamespace, options.keyPrefix),
